...
namespace EMS_TEST_SIMULATOR
{
    public class Rail_DIO                        
    {
        ...
        public Thread inputThread; //= new Thread(new ThreadStart(Func_input_status)) { IsBackground = true };
        public Thread outputThread; //= new Thread(new ThreadStart(Func_output_status)) { IsBackground = true };
        ...
        public bool FASTECH_CONNECT() // 외부에서 이 함수를 호출하여 연결 시작
        {
            int nRtn = 0;
            ...
            //if (Fastech_connect) return; // 이미 연결되어 있다면 중복 실행 방지
        ...
        public void IO_TEST()
        {
            // 입력/출력 상태를 감시/제어할 별도 스레드 시작


            inputThread = new Thread(new ThreadStart(Func_input_status)) { IsBackground = true };
            outputThread = new Thread(new ThreadStart(Func_output_status)) { IsBackground = true };
